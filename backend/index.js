...
const saveAnalytics = (data) => fs.writeFileSync(ANALYTICS_FILE, JSON.stringify(data, null, 2));
...
app.post('/analytics/log', (req, res) => {
  try {
    ...
    if (!analytics.sessions[session_id]) {
      analytics.sessions[session_id] = { steps: [], inputs: {}, startTime: timestamp, confidence_rating: null, confusion: null };
    }
    
    analytics.sessions[session_id].steps.push({ event, timestamp, data });
...
app.post('/feedback', (req, res) => {
  try {
    ...
    const analytics = loadAnalytics();
    if (!analytics.sessions[session_id]) return res.status(404).json({ error: "Session not found" });

    analytics.sessions[session_id].confidence_rating = confidence;
    analytics.sessions[session_id].confusion = confusion || null;
    saveAnalytics(analytics);
    res.json({ success: true });
...
app.listen(PORT, '0.0.0.0', () => log(`Backend running on ${PORT}`));
